...
constexpr uint32_t MAGIC = 0x58525353;
constexpr uint8_t VERSION = 1;
constexpr uint32_t PHASE_DURATION_MS = 30000;
constexpr uint32_t SEND_INTERVAL_MS = 100;
constexpr int8_t TX_POWER_LEVELS_QDBM[] = {34, 52, 68, 80};
...
struct __attribute__((packed)) SweepPacket {
  uint32_t magic;
  uint8_t version;
  uint8_t deviceId;
...
static_assert(sizeof(SweepPacket) == 20, "SweepPacket layout changed");
...
void onDataReceived(const esp_now_recv_info_t *info, const uint8_t *data,
                    int length) {
  if (length != sizeof(SweepPacket) || info == nullptr ||
  ...
  USBSerial.printf(
      "{\"type\":\"espnow_power_sample\",\"receiver_device_id\":%u,"
      "\"sender_device_id\":%u,\"sender_phase\":%u,\"seq\":%lu,"
      "\"requested_power_qdbm\":%d,\"applied_power_qdbm\":%d,"
      "\"rssi_dbm\":%d,\"channel\":%u,\"sender_uptime_ms\":%lu}\n",
      deviceId, packet.deviceId, packet.phaseIndex,
...
void applyPhase(uint8_t nextPhase) {
  phaseIndex = nextPhase % (sizeof(TX_POWER_LEVELS_QDBM) /
                            sizeof(TX_POWER_LEVELS_QDBM[0]));
  const int8_t requested = TX_POWER_LEVELS_QDBM[phaseIndex];
  if (esp_wifi_set_max_tx_power(requested) != ESP_OK ||
      esp_wifi_get_max_tx_power(&appliedPowerQdbm) != ESP_OK) {
    stopWithError("wifi_tx_power_failed");
  }
  phaseStartedAtMs = millis();
  USBSerial.printf(
      "{\"type\":\"espnow_power_phase\",\"device_id\":%u,"
      "\"phase\":%u,\"duration_ms\":%lu,"
      "\"requested_power_qdbm\":%d,\"applied_power_qdbm\":%d,"
      "\"send_failures\":%lu}\n",
      deviceId, phaseIndex, static_cast<unsigned long>(PHASE_DURATION_MS),
      requested, appliedPowerQdbm,
...
void loop() {
  ...
  packet.requestedPowerQdbm = TX_POWER_LEVELS_QDBM[phaseIndex];
